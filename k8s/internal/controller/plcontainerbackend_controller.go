...
func (r *PLContainerBackendReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if plcBackend.Status.Status == "" {
		...
		if _, err := ctrl.CreateOrUpdate(ctx, r.Client, &newPod, func() error {
			...
			log.Error(err, "failed to create pod", "pod", newPod)
	...
	// check if backend is deleting
	switch plcBackend.Status.Status {
	case plcontainerv1.BackendStatusCompleted:
		r.Client.Delete(ctx, &plcBackend)
		return ctrl.Result{}, nil
	case plcontainerv1.BackendStatusDeleting:
		if plcBackend.CreationTimestamp.Add(time.Minute).Before(time.Now()) {
			r.Client.Delete(ctx, &plcBackend)
			return ctrl.Result{}, nil
		}
	}

	if plcBackend.Status.BackendPodName == "" {
		// pod does not create for now, skip
		return ctrl.Result{}, nil
	}

	var pod corev1.Pod
	if err := r.Client.Get(ctx, client.ObjectKey{
		Namespace: plcBackend.Namespace,
		Name:      plcBackend.Status.BackendPodName,
	}, &pod); err != nil {
		// pod does not create for now, skip
		return ctrl.Result{}, err
	}

	// copy pod status to backend status
	...
	case corev1.PodRunning:
		...
		case plcontainerv1.NetworkModeClusterIP:
			...
			plcBackend.Status.BackendPort = pod.Spec.Containers[0].Ports[0].ContainerPort
		}
	case corev1.PodSucceeded:
		plcBackend.Status.Status = plcontainerv1.BackendStatusCompleted
		result = ctrl.Result{Requeue: true}
	case corev1.PodFailed:
		fallthrough
	case corev1.PodUnknown:
		plcBackend.Status.Status = plcontainerv1.BackendStatusDeleting
		result = ctrl.Result{RequeueAfter: time.Minute * 1}
	}

	// update plcBackend status
	if err := r.Status().Update(ctx, &plcBackend); err != nil {
		log.Error(err, "failed to update plcBackend status")
		return result, err
	}
...
func (r *PLContainerBackendReconciler) AllocNewPod(spec *plcontainerv1.PLContainerBackend) corev1.Pod {
	...
	// more label
	{
		ret.Labels["greenplum.org/segindex"] = "seg" + strconv.Itoa(int(spec.Spec.Segindex))
	...
	// volume mount
	{
		...
		for i := range hosts {
			...
			name := strings.ReplaceAll(container, "/", "-")
	...
	// core container part, cmdline, env, res and networking
	{
		...
		if spec.Spec.CpuShare != 0 {
			cpu := int64(spec.Spec.CpuShare) // mcore
			res.Limits[corev1.ResourceCPU] = *resource.NewScaledQuantity(cpu, resource.Milli)
		}
		if spec.Spec.MemoryMB != 0 {
			mem := int64(spec.Spec.MemoryMB)
			res.Limits[corev1.ResourceMemory] = *resource.NewScaledQuantity(mem, resource.Mega)
		}
...
func (r *PLContainerBackendReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		...
		for i := range backends.Items {
			...
			s := backend.Status.Status
			if s == plcontainerv1.BackendStatusCompleted || s == plcontainerv1.BackendStatusDeleting {
