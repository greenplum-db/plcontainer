...
int
plc_process_create_container(runtimeConfEntry *conf, char **name, int container_id, char **uds_dir)
{
	(void)(name);
	bool  has_error;
	...
	else if (pid == 0)
	{
		...
		// add more environment variables needed.
		char *const env[] = {
		        "USE_CONTAINER_NETWORK=false", "LOCAL_PROCESS_MODE=1", uid_string, gid_string, NULL,
		};
...
int
plc_process_start_container(const char *name)
{
	int res = 0;
...
int
plc_process_kill_container(const char *name)
{
	int res = 0;
	int pid = atoi(name);
	kill(pid, SIGKILL);
...
int
plc_process_inspect_container(const char *name, char **element, plcInspectionMode type)
{
	int res  = 0;
	*element = palloc(64);
...
int
plc_process_wait_container(const char *name)
{
	int res = 0;
	int pid = atoi(name);
	waitpid(pid, &res, 0);
...
int
plc_process_delete_container(const char *name)
{
	int res = 0;
	int pid = atoi(name);
	kill(pid, SIGKILL);
