...
static void
plcCurlBufferFree(plcCurlBuffer *buf)
{
	if (buf != NULL)
	{
...
static size_t
plcCurlCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
...
static plcCurlBuffer *
plcCurlRESTAPICall(plcCurlCallType cType, char *url, char *body)
{
	CURL          *curl;
	CURLcode       res;
	...
	if (log_min_messages <= DEBUG1) curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L);
	...
	if (res != CURLE_OK)
	{
		...
		elapsed_us = ((uint64)end_time.tv_sec) * 1000000 + end_time.tv_usec - ((uint64)start_time.tv_sec) * 1000000 -
		             start_time.tv_usec;

		snprintf(backend_error_message, sizeof(backend_error_message),
		         "PL/Container libcurl returns code %d, error '%s'", res, (len > 0) ? errbuf : curl_easy_strerror(res));
		buffer->status = -1;
...
int
plc_docker_start_container(const char *name)
{
	plcCurlBuffer *response = NULL;
...
int
plc_docker_kill_container(const char *name)
{
	plcCurlBuffer *response = NULL;
...
int
plc_docker_inspect_container(const char *name, char **element, plcInspectionMode type)
{
	plcCurlBuffer *response = NULL;
	...
	if (res < 0)
	{
		snprintf(backend_error_message, sizeof(backend_error_message), "Failed to inspect the container.");
		goto cleanup;
	}
...
int
plc_docker_wait_container(const char *name)
{
	plcCurlBuffer *response = NULL;
...
int
plc_docker_delete_container(const char *name)
{
	plcCurlBuffer *response = NULL;
...
int
plc_docker_list_container(char **result, int dbid)
{
	plcCurlBuffer *response = NULL;
...
int
plc_docker_get_container_state(const char *name, char **result)
{
	plcCurlBuffer *response = NULL;
...
static int
docker_inspect_string(char *buf, char **element, plcInspectionMode type)
{
	int                 i;
