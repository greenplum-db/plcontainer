...
init_runtime_configurations()
{
	 ...
	 */
	rumtime_conf_table =
	        hash_create("runtime configuration hash", MAX_EXPECTED_RUNTIME_NUM, &hash_ctl, HASH_ELEM | HASH_FUNCTION);
...
static void
parse_runtime_configuration(xmlNode *node)
{
	...
	PG_TRY();
	{
		...
		{
			if (cur_node->type == XML_ELEMENT_NODE && xmlStrcmp(cur_node->name, (const xmlChar *)"id") == 0)
			{
				if (id_num++ > 0)
				{
		...
		conf_entry =
		        (runtimeConfEntry *)hash_search(rumtime_conf_table, (const void *)runtime_id, HASH_ENTER, &foundPtr);
			...
			if (cur_node->type == XML_ELEMENT_NODE)
			{
				...
				if (xmlStrcmp(cur_node->name, (const xmlChar *)"setting") == 0)
				{
					...
					if (value != NULL)
					{
						...
						{
							plc_elog(ERROR,
							         "SETTING element <use_container_logging> only accepted \"yes\" or"
							         "\"no\" only, current string is %s",
							         value);
						}
						xmlFree((void *)value);
						value = NULL;
					...
					if (value != NULL)
					{
						...
						{
							plc_elog(ERROR,
							         "container memory size couldn't be equal or less than 0, current string is %s",
							         value);
						}
						else
						{
					...
					if (value != NULL)
					{
						...
						{
							plc_elog(ERROR,
							         "container cpu share couldn't be equal or less than 0, current string is %s",
							         value);
						}
						else
						{
					...
					if (value != NULL)
					{
						...
						{
							plc_elog(ERROR,
							         "SETTING element <resource_group_id> must be a resource group id in greenplum. "
						...
						{
							plc_elog(ERROR,
							         "SETTING element <resource_group_id> must be a resource group with memory_auditor "
							         "type cgroup.");
						}
#endif
					...
					if (!validSetting)
					{
						plc_elog(ERROR, "Unrecognized setting options, please check the configuration file: %s",
						         conf_entry->runtimeid);
					}
				}
				...
				if (processed == 0)
				{
					plc_elog(ERROR, "Unrecognized element '%s' inside of container specification", cur_node->name);
				}
			}
		}
		...
		if (image_num > 1)
		{
			plc_elog(ERROR, "There are more than one 'image' subelement in a runtime element %s",
			         conf_entry->runtimeid);
		}
		else if (image_num < 1)
		{
		...
		if (command_num > 1)
		{
			plc_elog(ERROR, "There are more than one 'command' subelement in a runtime element %s",
			         conf_entry->runtimeid);
		}
		...
		if (num_shared_dirs > 0)
		{
			...
			for (cur_node = node->children; cur_node; cur_node = cur_node->next)
			{
				    ...
				    xmlStrcmp(cur_node->name, (const xmlChar *)"shared_directory") == 0)
				{
					...
					{
						plc_elog(ERROR,
						         "Configuration tag 'shared_directory' has a mandatory element"
						         " 'host' that is not found: %s",
						         conf_entry->runtimeid);
					}
					...
					for (j = 0; j < i; j++)
					{
						...
						{
							plc_elog(ERROR,
							         "Container path cannot be the same in 'shared_directory' element "
							         "in the runtime %s",
							         conf_entry->runtimeid);
						}
					}
					...
					{
						plc_elog(ERROR,
						         "Directory access mode should be either 'ro' or 'rw', but passed value is '%s': %s",
						         value, conf_entry->runtimeid);
					}
					xmlFree((void *)value);
					value = NULL;
					i += 1;
...
static void
get_runtime_configurations(xmlNode *node)
{
	xmlNode *cur_node = NULL;
	...
	{
		plc_elog(ERROR,
		         "Wrong XML configuration provided. Expected 'configuration'"
		         " as root element, got '%s' instead",
		         node->name);
	}
	...
	for (cur_node = node->children; cur_node; cur_node = cur_node->next)
	{
		if (cur_node->type == XML_ELEMENT_NODE && xmlStrcmp(cur_node->name, (const xmlChar *)"runtime") == 0)
		{
...
static void
free_runtime_conf_entry(runtimeConfEntry *entry)
{
	int i;

	if (entry->image) pfree(entry->image);
	if (entry->command) pfree(entry->command);

	for (i = 0; i < entry->nSharedDirs; i++)
	{
		if (entry->sharedDirs[i].container) pfree(entry->sharedDirs[i].container);
		if (entry->sharedDirs[i].host) pfree(entry->sharedDirs[i].host);
	}
	if (entry->sharedDirs) pfree(entry->sharedDirs);
}
...
print_runtime_configurations()
{
	...
	if (rumtime_conf_table != NULL)
	{
		...
		while ((conf_entry = (runtimeConfEntry *)hash_seq_search(&hash_status)) != NULL)
		{
			...
			{
				plc_elog(INFO, "    shared directory from host '%s' to container '%s'", conf_entry->sharedDirs[j].host,
...
static int
plc_refresh_container_config(bool verbose)
{
...
#ifdef PLC_PG
	if ((env_str = getenv("PGDATA")) == NULL) plc_elog(ERROR, "PGDATA is not set");
...
runtimeConfEntry *
plc_get_runtime_configuration(char *runtime_id)
{
	int               res   = 0;
...
bool
plc_check_user_privilege(char *roles)
{
	List     *elemlist;
	ListCell *l;
	Oid       currentUserOid;
