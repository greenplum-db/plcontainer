...
static plcMsgResult *
create_sql_result(bool isSelect)
{
	plcMsgResult *result;
	uint32        i, j;
	...
	else
	{
		...
		for (i = 0; i < result->rows; i++)
		{
			...
			for (j = 0; j < result->cols; j++)
			{
				origval = SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, j + 1, &isnull);
				if (isnull)
				{
...
static plcMsgRaw *
create_prepare_result(int64 pplan, plcDatatype *type, int nargs)
{
	plcMsgRaw   *result;
	unsigned int offset;
	...
	*((int32 *)(result->data + offset)) = nargs;
	offset += sizeof(int32);
	if (nargs > 0) memcpy(result->data + offset, type, nargs * sizeof(plcDatatype));

	return result;
}

static plcMsgRaw *
create_unprepare_result(int32 retval)
{
	plcMsgRaw *result;
...
static int
search_pplan(plcConn *conn, int64 pplan)
{
	int                 i;
	...
	{
		if (pplans[i].pplan == pplan) return i;
	}
...
static int
insert_pplan(plcConn *conn, int64 pplan)
{
	int                 slot;
	struct pplan_slots *pplans;
	...
	if (slot >= 0)
	{
		plc_elog(DEBUG1, "Inserting pplan 0x%llx at slot %d for container %d", (long long)pplan, slot,
		         conn->container_slot);
...
static int
delete_pplan(plcConn *conn, int64 pplan)
{
	int                 slot;
	struct pplan_slots *pplans;
	...
	{
		pplans = conn->pplans;
		plc_elog(DEBUG1, "Removing pplan 0x%llx at slot %d, for container %d", (long long)pplan, slot,
		         conn->container_slot);
		pplans[slot].pplan         = 0;
...
static int
free_plc_plan(plcConn *conn, int64 pplan)
{
	plcPlan *plc_plan;
	int      retval;
	...
	if (plc_plan->argOids) pfree(plc_plan->argOids);
	if (plc_plan->plan) retval = SPI_freeplan(plc_plan->plan);
	pfree(plc_plan);
...
void
deinit_pplan_slots(plcConn *conn)
{
	int                 i;
...
plcMessage *
handle_sql_message(plcMsgSQL *msg, plcConn *conn, plcProcInfo *pinfo)
{
	int                    i, retval;
		...
		switch (msg->sqltype)
		{
			...
			case SQL_TYPE_PEXECUTE:
				if (msg->sqltype == SQL_TYPE_PEXECUTE)
				{
					...
					{
						plc_elog(ERROR,
						         "argument number wrong for execute with plan: "
					 ...
					 */
					retval = SPI_execute_plan(plc_plan->plan, values, nulls, pinfo->fn_readonly, (long)msg->limit);
				...
				switch (retval)
				{
					...
					default:
						plc_elog(ERROR,
						         "Cannot handle sql ('%s') with fn_readonly (%d) "
						         "and limit (" INT64_FORMAT "). Detail: %s",
						         msg->statement, pinfo->fn_readonly, msg->limit, SPI_result_code_string(retval));
						break;
				}
				break;
			case SQL_TYPE_PREPARE:
				...
				for (i = 0; i < msg->nargs; i++)
				{
					...
					{
						plc_elog(ERROR, "prepare type is bad, unexpected prepare sql type %d", msg->args[i].type.type);
					}
				...
				else
				{
					...
					plc_elog(LOG, "SPI_prepare() fails for '%s', with %d arguments: %s", msg->statement,
					         plc_plan->nargs, SPI_result_code_string(SPI_result));
				}
