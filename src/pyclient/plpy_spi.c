...
#include "../common/spiexceptions.h"
	{NULL, NULL, 0}
};

char * unpack_sql_state(int sql_state);
static PyObject *PLy_spi_exceptions = NULL;
#endif
...
PLy_generate_spi_exceptions(PyObject *mod, PyObject *base)
{
	...
	for (i = 0; exception_map[i].name != NULL; i++)
	{
		...
		if (dict == NULL)
			plc_elog(ERROR, "could not generate SPI exceptions");
		...
		PyDict_SetItemString(PLy_spi_exceptions, unpack_sql_state(exception_map[i].sqlstate), exc);
	}
}
#endif
