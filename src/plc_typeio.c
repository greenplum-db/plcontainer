...
static void fill_type_info_inner(FunctionCallInfo fcinfo, Oid typeOid, plcTypeInfo *type, bool isArrayElement,
                                 bool isUDTElement);
...
fill_type_info_inner(FunctionCallInfo fcinfo, Oid typeOid, plcTypeInfo *type, bool isArrayElement, bool isUDTElement)
{
	...
	if (!HeapTupleIsValid(typeTup)) plc_elog(ERROR, "cache lookup failed for type %u", typeOid);
	...
	get_type_io_data(typeOid, IOFunc_input, &type->typlen, &type->typbyval, &type->typalign, &dummy_delim, &typioparam,
	                 &type->input);
...
void
fill_type_info(FunctionCallInfo fcinfo, Oid typeOid, plcTypeInfo *type)
{
	fill_type_info_inner(fcinfo, typeOid, type, false, false);
}

void
copy_type_info(plcType *type, plcTypeInfo *ptype)
{
...
void
free_type_info(plcTypeInfo *type)
{
	int i = 0;
...
static char *
plc_datum_as_int1(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
...
static char *
plc_datum_as_int2(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
...
static char *
plc_datum_as_int4(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
...
static char *
plc_datum_as_int8(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
...
static char *
plc_datum_as_float4(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
	char *out        = (char *)pmalloc(4);
...
static char *
plc_datum_as_float8(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
	char *out        = (char *)pmalloc(8);
...
static char *
plc_datum_as_float8_numeric(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
	char *out = (char *)pmalloc(8);
...
static char *
plc_datum_as_text(Datum input, plcTypeInfo *type)
{
	return DatumGetCString(OidFunctionCall3(type->output, input, type->typelem, type->typmod));
}

static char *
plc_datum_as_bytea(Datum input, pg_attribute_unused() plcTypeInfo *type)
{
...
static char *
plc_datum_as_array(Datum input, plcTypeInfo *type)
{
...
static void
plc_backend_array_free(plcIterator *iter)
{
	plcArrayMeta *meta;
...
static rawdata *
plc_backend_array_next(plcIterator *self)
{
	plcTypeInfo        *subtyp;
	rawdata	        *res;
	plcPgArrayPosition *pos;
...
*/
static char *
plc_datum_as_udt(Datum input, plcTypeInfo *type)
{
...
static Datum
plc_datum_from_int1(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return BoolGetDatum(*((bool *)input));
}

static Datum
plc_datum_from_int2(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return Int16GetDatum(*((int16 *)input));
}

static Datum
plc_datum_from_int4(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return Int32GetDatum(*((int32 *)input));
}

static Datum
plc_datum_from_int8(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return Int64GetDatum(*((int64 *)input));
}

static Datum
plc_datum_from_float4(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return Float4GetDatum(*((float4 *)input));
}

static Datum
plc_datum_from_float8(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	return Float8GetDatum(*((float8 *)input));
}

static Datum
plc_datum_from_float8_numeric(char *input, pg_attribute_unused() plcTypeInfo *type)
{
...
static Datum
plc_datum_from_text(char *input, plcTypeInfo *type)
{
	return OidFunctionCall3(type->input, CStringGetDatum(input), type->typelem, type->typmod);
}

static Datum
plc_datum_from_text_ptr(char *input, plcTypeInfo *type)
{
	return OidFunctionCall3(type->input, CStringGetDatum(*((char **)input)), type->typelem, type->typmod);
}

static Datum
plc_datum_from_bytea(char *input, pg_attribute_unused() plcTypeInfo *type)
{
	int    size   = *((int *)input);
...
static Datum
plc_datum_from_bytea_ptr(char *input, plcTypeInfo *type)
{
	return plc_datum_from_bytea(*((char **)input), type);
}

static Datum
plc_datum_from_array(char *input, plcTypeInfo *type)
{
	Datum        dvalue;
	Datum       *elems;
	...
	for (i = 0; i < arr->meta->ndims; i++) lbs[i] = 1;
	...
	array = construct_md_array(elems, arr->nulls, arr->meta->ndims, arr->meta->dims, lbs, subType->typeOid,
	                           subType->typlen, subType->typbyval, subType->typalign);
...
static Datum
plc_datum_from_udt(char *input, plcTypeInfo *type)
{
	TupleDesc    desc;
	HeapTuple    tuple;
	Datum       *values;
...
static Datum
plc_datum_from_udt_ptr(char *input, plcTypeInfo *type)
{
	return plc_datum_from_udt(*((char **)input), type);
}

plcDatatype
plc_get_datatype_from_oid(Oid oid)
{
	plcDatatype dt;
