...
is_write_log(int elevel, int log_min_level)
{
	...
	{
		if (log_min_level == LOG || log_min_level <= ERROR) return 1;
	}
	else if (log_min_level == LOG)
	{
		/* elevel != LOG */
		if (elevel >= FATAL) return 1;
	}
	/* Neither is LOG */
...
pmalloc(size_t size)
{
	...
	if (addr == NULL) plc_elog(ERROR, "Fail to allocate %ld bytes", (unsigned long)size);
	return addr;
}
...
static void
set_signal_handler(int signo, int sigflags, signal_handler func)
{
	int              ret;
	struct sigaction sa;
	...
	if (ret < 0)
	{
		plc_elog(ERROR, "sigaction(%d with flag 0x%x) failed: %s", signo, sigflags, strerror(errno));
		return;
	}
