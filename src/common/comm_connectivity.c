...
plc_gettimeofday(struct timeval *tv)
{
	...
	if (retval < 0) plc_elog(ERROR, "Failed to get time: %s", strerror(errno));
}
 ...
 */
static ssize_t
plcSocketRecv(plcConn *conn, void *ptr, size_t len)
{
	ssize_t        sz         = 0;
	int            time_count = 0;
	int            intr_count = 0;
	...
	while ((sz = recv(conn->sock, ptr, len, 0)) < 0)
	{
...
#endif
		if (errno == EINTR && intr_count++ < 5) continue;
		if (errno == EAGAIN || errno == EWOULDBLOCK)
		{
			...
			else
			{
				...
				{
					plc_elog(ERROR, "rx timeout (%ds > %ds)", (int)(end_ts.tv_sec - start_ts.tv_sec),
					         conn->rx_timeout_sec);
					return -1;
				}
			}
 ...
 */
static ssize_t
plcSocketSend(plcConn *conn, const void *ptr, size_t len)
{
	ssize_t sz;
	int     n = 0;
 ...
 */
static int
plcBufferMaybeFlush(plcConn *conn, bool isForse)
{
	 ...
	 */
	if (buf->bufSize - buf->pEnd < PLC_BUFFER_MIN_FREE || buf->pEnd - buf->pStart > PLC_BUFFER_SIZE || isForse)
	{
		...
		while (buf->pStart < buf->pEnd)
		{
			...
			sent = plcSocketSend(conn, buf->data + buf->pStart, buf->pEnd - buf->pStart);
			if (sent < 0)
			{
				plc_elog(LOG,
				         "plcBufferMaybeFlush: Socket write failed, send "
 ...
 */
static void
plcBufferMaybeReset(plcConn *conn, int bufType)
{
 ...
 */
static int
plcBufferMaybeResize(plcConn *conn, int bufType, size_t bufAppend)
{
	plcBuffer *buf = conn->buffer[bufType];
	int        dataSize;
	int        newSize;
	...
	if (dataSize < buf->bufSize / 5 && buf->bufSize > PLC_BUFFER_SIZE)
	{
		...
		if (newBuffer == NULL)
		{
			...
			plc_elog(WARNING,
			         "plcBufferMaybeFlush: Cannot allocate %d bytes "
			         "for output buffer",
			         newSize);
			return 0;
		}
	...
	else if (buf->pEnd + (int)bufAppend > buf->bufSize - PLC_BUFFER_MIN_FREE)
	{
		...
		if (newBuffer == NULL)
		{
			plc_elog(ERROR, "plcBufferMaybeGrow: Cannot allocate %d bytes for buffer", newSize);
			return -1;
		}
		isReallocated = 1;
	}
	...
	if (isReallocated)
	{
		memcpy(newBuffer, buf->data + buf->pStart, (size_t)(buf->pEnd - buf->pStart));
		pfree(buf->data);
		buf->data    = newBuffer;
		buf->pEnd    = buf->pEnd - buf->pStart;
		buf->pStart  = 0;
 ...
 */
int
plcBufferAppend(plcConn *conn, char *srcBuffer, size_t nBytes)
{
	int        res = 0;
	plcBuffer *buf = conn->buffer[PLC_OUTPUT_BUFFER];
	...
	if (buf->bufSize - buf->pEnd < (int)nBytes)
	{
		...
		res = plcBufferMaybeResize(conn, PLC_OUTPUT_BUFFER, nBytes);
		if (res < 0) return res;
	}
 ...
 */
int
plcBufferRead(plcConn *conn, char *resBuffer, size_t nBytes)
{
 ...
 */
int
plcBufferReceive(plcConn *conn, size_t nBytes)
{
	int        res = 0;
	plcBuffer *buf = conn->buffer[PLC_INPUT_BUFFER];
	...
	if (buf->pEnd - buf->pStart < (int)nBytes)
	{
		...
		{
			recBytes = plcSocketRecv(conn, buf->data + buf->pEnd, buf->bufSize - buf->pEnd);
			if (recBytes <= 0)
			{
				return -1;
			}
...
plcConnect_inet(int port)
{
	...
	if (server == NULL)
	{
		close(sock);
		plc_elog(ERROR,
		         "PLContainer: Failed to call gethostbyname('localhost'):"
		         " %s",
		         hstrerror(h_errno));
		return NULL;
	}
	...
	memcpy(&((raddr).sin_addr.s_addr), (char *)server->h_addr_list[0], server->h_length);

	raddr.sin_port = htons(port);
	if (connect(sock, (const struct sockaddr *)&raddr, sizeof(raddr)) < 0)
	{
		char ipAddr[INET_ADDRSTRLEN];
		inet_ntop(AF_INET, &(raddr.sin_addr), ipAddr, INET_ADDRSTRLEN);
		plc_elog(DEBUG1, "PLContainer: Failed to connect to %s: %s", ipAddr, strerror(errno));
		goto err_out2;
	}
...
plcConnect_ipc(char *uds_fn)
{
	...
	{
		plc_elog(ERROR,
		         "PLContainer: The path for unix domain socket "
		         "connection is too long: %s",
		         uds_fn);
		return NULL;
	}
	...
	if (sock < 0)
	{
		plc_elog(ERROR, "PLContainer: Cannot create unix domain socket: %s", strerror(errno));
		return NULL;
	}
	...
	if (connect(sock, (const struct sockaddr *)&raddr, sizeof(raddr)) < 0)
	{
		plc_elog(DEBUG1, "PLContainer: Failed to connect to %s: %s", uds_fn, strerror(errno));
		goto err_out;
	}
