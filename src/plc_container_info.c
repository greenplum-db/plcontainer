...
list_running_containers(pg_attribute_unused() PG_FUNCTION_ARGS) {
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		attinmeta = TupleDescGetAttInMetadata(((ReturnSetInfo *)fcinfo->resultinfo)->expectedDesc);
...
#define		CONTAINER_MAX_NUM_ATTS 7
			...
			if (res >= 0) {
				...
				snprintf(values[4], 64, "%" PRId64, containerMemoryUsage);
...
containers_summary(pg_attribute_unused() PG_FUNCTION_ARGS) {
	return list_running_containers(fcinfo);
}
