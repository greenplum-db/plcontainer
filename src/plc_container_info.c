...
list_running_containers(pg_attribute_unused() PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		TupleDescInitEntry(tupdesc, (AttrNumber)1, "SEGMENT_ID", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)2, "CONTAINER_ID", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)3, "UP_TIME", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)4, "OWNER", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)5, "MEMORY_USAGE(KB)", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)6, "CPU_USAGE", TEXTOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber)7, "Current Running Function", TEXTOID, -1, 0);
	...
	while (1)
	{
		...
		if (call_cntr < max_calls)
		{
			...
			if (!json_object_object_get_ex(labelObj, "owner", &ownerObj))
			{
				...
				call_cntr++;
				plc_elog(LOG,
				         "failed to get json \"owner\" field. Maybe this container is not started by PL/Container");
				continue;
			}
			...
			if (strcmp(ownerStr, username) != 0 && superuser() == false)
			{
				funcctx->call_cntr++;
				call_cntr++;
				plc_elog(DEBUG1, "Current username %s (not super user) is not match conatiner owner %s, skip", username,
				         ownerStr);
				continue;
			}
...
plcontainer_shmem_startup(void)
{
	...
	udf_container_id_map = ShmemInitHash("plcontainer map entries between container id and udf name", MAX_UDF_ENTRIES,
	                                     MAX_UDF_ENTRIES, &hash_ctl, HASH_ELEM | HASH_FUNCTION);
...
void
del_containerid_entry(char *dockerid)
{
	bool found;
	char cid[PREFIX_CONTAINER_ID_LENGTH];
...
collect_running_containers(pg_attribute_unused() PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < cdb_pgresults.numResults; i++)
	{
		...
		if (PQresultStatus(pgresult) != PGRES_TUPLES_OK)
		{
			...
			ereport(ERROR, (errmsg("Unable to get container information from segment: %d", PQresultStatus(pgresult))));
		}
