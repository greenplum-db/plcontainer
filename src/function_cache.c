...
plcProcInfo *
function_cache_get(Oid funcOid)
{
	int          i;
	...
	for (i = 0; i < PLC_FUNCTION_CACHE_SIZE; i++)
	{
		if (plcFunctionCache[i] != NULL && plcFunctionCache[i]->funcOid == funcOid)
		{
...
void
function_cache_put(plcProcInfo *func)
{
	int          i;
	plcProcInfo *oldFunc;
