...
containers_cnt=$(ssh `psql -c 'select address from gp_segment_configuration where dbid=2' -t -A` docker ps -a --filter label=dbid=2 -q | wc -l)
if ((containers_cnt > 0)); then
    ssh `psql -c 'select address from gp_segment_configuration where dbid=2' -t -A` docker rm -f $(docker ps -a --filter label=dbid=2 -q)
fi
