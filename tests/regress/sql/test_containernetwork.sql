\! plcontainer runtime-add -r plc_python_shared_net -i ${CONTAINER_NAME_SUFFIX_PYTHON} -l python3 --setting enable_network=yes -s roles=$USER $(./data/auto_select_client.py -i ${CONTAINER_NAME_SUFFIX_PYTHON})
...
\! plcontainer runtime-add -r plc_python_shared_net -i ${CONTAINER_NAME_SUFFIX_PYTHON} -l python3 --setting enable_network=yes $(./data/auto_select_client.py -i ${CONTAINER_NAME_SUFFIX_PYTHON})
select * from plcontainer_refresh_config;
select * from access_network_ok();
